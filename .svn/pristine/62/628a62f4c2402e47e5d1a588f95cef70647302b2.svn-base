...
__date__ = '2017/5/9'
"""
牛车网的文章（通过excel模板）（下一页需要js渲染才能获取）
"""
...
def read_execel_element_config(name):
    ...
    next_page = ws['D4'].value
    return list_xpath_result, article_xpath_result, next_page


class Niuchewang_Parser(Parser):
    list_xpath_dic, article_xpath_dic, next_page = read_execel_element_config('niuchewang.xlsx')
        ...
        try:
            ...
            if self.list_xpath_dic.get('next_page'):
                ...
                if next_pages:
                    ...
                    next_page = self.next_page.replace('2', '{}')
                    for page_index in range(2, 11):
                        sent_kafka_message = copy.deepcopy(original_sent_kafka_message)
                        next_page_url = next_page.format(page_index)
                ...
                for key, xpath_str in self.list_xpath_dic.iteritems():
                    if key in ( 'id', 'data_source_name', 'data_source_id','next_page', 'article_list','excel'):
                        ...
                        if sent_kafka_message[key]:
                            sent_kafka_message[key].replace(" ", "").strip()
                yield sent_kafka_message
    ...
    def extract_author_text(self, author):
        if author:
            try:
                author = re.findall(ur'(?:作者：|发布人：|作者:|编辑：)[\s]*([\u4e00-\u9fa50-9a-z]+)[\s]*', author)[0].strip()
            except:
                author = author.strip()
            return author
        return None
