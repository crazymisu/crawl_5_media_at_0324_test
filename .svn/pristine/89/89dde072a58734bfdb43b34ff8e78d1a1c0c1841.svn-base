...
class Bjtvnews_Parser(Parser):
    ...
    def parse_list_page(self, response):
        try:
            ...
            article_list =  sel.xpath('//div[@class="list_ys"]/descendant::a')
            article = {}
            next_page = sel.xpath('//div[@class="pages"]/a[text()="%s"]/@href' % u'下一页').extract_first()

            for a in article_list:
                sent_kafka_message = copy.deepcopy(original_sent_kafka_message)

                article['url'] = a.xpath('./@href')[0].extract()
                article['domain'] = 'bjtvnews.com'
    ...
    def parse_detail_page(self, response):
        try:
            ...
            if response.status == 200:
                ...
                article_info = sel.xpath('//div[@class="wb_xwly"]/text()').extract()
                article_info_source = article_info[0].split(u'    ')[1].split(u'：')[1]
                pub_time = article_info[0].split(u'    ')[0].split(u'时间:')[1]
                tags = sel.xpath('//div[@class="wb_gjz"]/text()').extract()
                sent_kafka_message['publish_time'] = self.parse_toutiao_publish_time(pub_time)
                if tags:
                    sent_kafka_message['tags'] = tags[0].split(u'关键字：')[1].split(u',')
                ...
                content_html = sel.xpath('//div[@class="wb_nr"]')[0].extract()
