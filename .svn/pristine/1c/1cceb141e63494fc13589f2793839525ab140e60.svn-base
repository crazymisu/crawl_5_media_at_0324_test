...
class Zhongguoshangbao_Parser(Parser):
    list_xpath_dic, article_xpath_dic = read_execel_element_config('zhongguoshangbao.xlsx')
            ...
            for li_document in document_list:
                ...
                for key, xpath_str in self.list_xpath_dic.iteritems():
                    if key in ( 'id', 'data_source_name', 'data_source_id','next_page', 'article_list','excel'):
                        ...
                        for item in value:
                            if len(item.strip()) > 1 and item.find(u'标签') == -1 and item.find(u'关键词') == -1:
                                sent_kafka_message[key].append(item.strip().replace(' ', ''))
                    elif key == 'small_img_location':
                        if li_document.xpath('./descendant::img/@src').extract_first():
