...
class Genwoshijia_Spider(Parser):
    list_xpath_dic, article_xpath_dic = read_execel_element_config('genwoshijia.xlsx')
                ...
                for key, xpath_str in self.list_xpath_dic.iteritems():
                    if key in ( 'id', 'data_source_name', 'data_source_id','next_page', 'article_list','excel'):
                        ...
                        if li_document.xpath(xpath_str[list_area + 1:]).extract_first():
                            img_path = li_document.xpath(xpath_str[list_area + 1:] + '/@style').extract_first()
                            small_img_location = re.findall(ur'url\((.*?)\)', img_path)[0]
