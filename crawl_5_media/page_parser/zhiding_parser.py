...
class ZhiDing_Parser(Parser):
    ...
    def parse_list_page(self, response):
        try:
            ...
            article_list = sel.xpath('//div[@class="qu_left"]/div[contains(@class,"qu_loop")]')
            article = {}
            next_page_href = sel.xpath('//div[@class="pagebar"]/a[text()="%s"]/@href' % u'下一页').extract()[0]
            ...
            for a in article_list:
                ...
                article['url'] = a.xpath('./div[@class="qu_ims"]/a/@href')[0].extract()
                article['domain'] = 'zhiding.cn'
                article['title'] = a.xpath('./div[@class="qu_ims"]/a/@title')[0].extract()
                article['publish_time'] = a.xpath('./div[@class="qu_times"]/text()')[0].extract()
                if a.xpath('./div[@class="qu_ims"]/a/img'):
                    small_img_url = a.xpath('./div[@class="qu_ims"]/a/img/@src')[0].extract()
    ...
    def parse_detail_page(self, response):
        try:
            ...
            if response.status == 200:
                ...
                desc = sel.xpath('//div[@class="qu_zhao"]/text()')[0].extract()
                if desc:
                    sent_kafka_message['desc'] = desc
                else:
                    sent_kafka_message['desc'] = None

                article_info = sel.xpath('//div[@class="qu_zuo"]/descendant::*/text()').extract()
                author = re.findall('%s' % u'作者：(.*?) ',article_info[0])
                article_info_source = re.findall('%s' % u'来源：(.*?) ',article_info[0])
                tags = [x.replace(u'\r\n',u'') for x in article_info[2::] if x!= u' ']
                if tags:
                ...
                content_html = sel.xpath('//div[@class="qu_ocn"]')[0].extract()
