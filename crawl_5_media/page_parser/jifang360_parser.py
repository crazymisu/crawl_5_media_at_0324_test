...
class Jifang360_Parser(Parser):
    ...
    def parse_list_page(self, response):
        try:
            ...
            article_list = sel.xpath('//div[@class="news_list"]/ul/li[1]/a')
            article = {}
            next_page_href = sel.xpath('//div[@class="news_list"]/div/a[@title="%s"]/@href' % u'下一页').extract()[0]
            ...
            for a in article_list:
                ...
                article['domain'] = 'jifang360.com'
                article['title'] = a.xpath('./text()')[0].extract()
    ...
    def parse_detail_page(self, response):
        try:
            ...
            if response.status == 200:
                ...
                article_info = sel.xpath('//div[@class="times"]/text()').extract()
                sent_kafka_message['publish_time'] = self.parse_toutiao_publish_time(article_info[1])
                desc = sel.xpath('//div[@class="content"]/table[1]/tbody/tr/td/text()')
                if desc:
                    sent_kafka_message['desc'] = desc[0].extract()
                article_info_source = article_info[0].split(u'：')[1].split(u' ')[0]
                author = sel.xpath('//div[@class="times"]/a/text()')[0].extract()
                ...
                sent_kafka_message['authorized'] = u'版权声明：凡本站原创文章，未经授权，禁止转载，否则追究法律责任'
    ...
    def parse_article_content(self, content_html,pub_date):
        ...
        if content_html:
            ...
            for p in p_content_document_list:
                if isinstance(p,bs4.Tag) and p.name!='table':
                    if p.findChild('img'):
