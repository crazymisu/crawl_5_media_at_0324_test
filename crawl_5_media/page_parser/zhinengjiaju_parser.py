...
class Zhinengjiaju_Parser(Parser):
    list_xpath_dic, article_xpath_dic = read_execel_element_config('zhinengjiaju.xlsx')
        ...
        try:
            ...
            for key, xpath_str in self.article_xpath_dic.iteritems():
                if key == 'parsed_content':
                    ...
                    parsed_result = self.extract_article_parsed_content(content_document,None,None,response)
                    ...
                    sent_kafka_message[key] = sent_kafka_message[key] if sent_kafka_message.get(key)  else  self.extract_number_count(
                        response.xpath(xpath_str + '/text()').extract_first())
                elif key == 'url':
                    pass
                else:
