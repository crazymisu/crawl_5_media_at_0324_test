...
class PchomeParser(Parser):
    ...
    def parse_list_page(self, response):
        ...
        try:
            ...
            if response.url == 'http://biz.pchome.net/':
                details = response.xpath('//div[@class="con2"]/ul/li')
                for detail in details:
                    sent_kafka_message = copy.deepcopy(original_sent_kafka_message)
                    sent_kafka_message['url'] = detail.xpath('./a/@href').extract()[0]  # 详情页url
                    sent_kafka_message['title'] = detail.xpath('./h4//text()').extract()[0]  # 标题
                    sent_kafka_message['desc'] = detail.xpath('./p/text()').extract_first()  # 简介

                    small_img_url = detail.xpath('.//img/@src').extract_first()
                    if small_img_url:
                        small_img_location, check_flag = self.get_small_img_location(small_img_url, None)
                    ...
                    sent_kafka_message['url_domain'] = 'pchome.com'
                    sent_kafka_message['parse_function'] = 'parse_detail_page'
                    yield sent_kafka_message

            else:
                details = response.xpath('//ul[@class="cms-list"]/li')
                for detail in details:
                    sent_kafka_message = copy.deepcopy(original_sent_kafka_message)
                    sent_kafka_message['url'] =  detail.xpath('.//h3/a/@href').extract()[0]  # 详情页url
                    sent_kafka_message['title'] = detail.xpath('.//h3/a/text()').extract()[0]  # 标题
                    sent_kafka_message['desc'] = detail.xpath('.//p/text()').extract_first()  # 简介
    ...
    def parse_detail_page(self, response):
        try:
            ...
            sent_kafka_message['info_source'] = response.xpath('//a[@id="source_baidu"]/text()').extract_first()
            sent_kafka_message['author'] = response.xpath('//span[@id="editor_baidu"]/text()').extract_first()[3:].strip()
            sent_kafka_message['publish_time'] = response.xpath('//span[@id="pubtime_baidu"]/text()').extract_first().strip()

            content_html = response.xpath('//div[@class="cms-article-text"]').extract_first()
            content_html = content_html.replace("<script>write_ad('article_mid_pip');</script>", '')
