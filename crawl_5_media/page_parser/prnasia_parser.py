...
class PrnasiaParser(Parser):
    ...
    def parse_list_page(self, response):
        ...
        try:
            ...
            urls = response.xpath('//div[@class="presscolumn"]')  # 每页50个
            for url in urls:
                sent_kafka_message = copy.deepcopy(original_sent_kafka_message)
                # 详情页url
                sent_kafka_message['url'] = 'http://www.prnasia.com/' + url.xpath('./h3/a/@href').extract()[0]
                # 标题
                sent_kafka_message['title'] = url.xpath('./h3/a/text()').extract()[0]
                # 发布时间
                sent_kafka_message['publish_time'] = url.xpath('./span[@class="timestamp"]/text()').extract()[0].strip()
                # 小图，很少出现
                ...
                sent_kafka_message['url_domain'] = 'prnasia.com'
    ...
    def parse_detail_page(self, response):
        try:
            ...
            content_html_list = response.xpath('//div[@id="dvContent"]').extract()  # 文章块
            content_html = ''.join(content_html_list)  # 转成str
            content_html = '<div>' + re.search(r'<p>.*</p>', content_html, re.S).group(0) + '</div>'  # 去除css，得到带标签的正文
    ...
    def get_small_img_location(self, response, publish_time):
        try:
            src = response.xpath('//div[@class="presscolumn"]//img/@src').extract()[0]
        except:
            return None, True
