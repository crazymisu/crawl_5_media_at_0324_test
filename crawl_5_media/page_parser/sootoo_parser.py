...
class SootooParser(Parser):
    ...
    def parse_list_page(self, response):
        ...
        try:
            ...
            details = response.xpath('//div[@class="t10_mlblk"]/p')
            for detail in details:
                sent_kafka_message = copy.deepcopy(original_sent_kafka_message)
                sent_kafka_message['url'] =  'http://www.sootoo.com' + detail.xpath('.//a[2]/@href').extract()[0]
                # /content/670954.shtml
                sent_kafka_message['title'] = detail.xpath('.//a[2]/text()').extract_first().strip()  # 标题

                sent_kafka_message['response_url'] = sent_kafka_message['url']
                sent_kafka_message['url_domain'] = 'sootoo.com'
    ...
    def parse_detail_page(self, response):
        try:
            ...
            # 2017年05月9日9:43
            publish_time = response.xpath('//div[@class="t11_info"]//span[1]/text()').extract_first()
            sent_kafka_message['publish_time'] = self.get_time(publish_time)
            sent_kafka_message['author'] = response.xpath('//div[@class="t11_info"]//a[1]/text()').extract_first()
            sent_kafka_message['info_source'] = response.xpath('//div[@class="t11_info"]//a[2]/text()').extract_first()

            content_html = response.xpath('//div[@id="content"]').extract()[0]
    ...
    def get_time(self,source_time):
        if u'小时前' in source_time:
            ...
            dt = str((datetime.now() - datetime.timedelta(hours=int(time_back)))).split(r'.')[0]
        elif u'分钟前' in source_time:
            time_back = source_time.split(u'分钟前')[0]
            dt = str((datetime.now() - datetime.timedelta(minutes=int(time_back)))).split(r'.')[0]
        else:
