...
class ZNgonghui_Parser(Parser):
    ...
    def parse_list_page(self, response):
        try:
            ...
            article_list =  sel.xpath('//div[@class="arcList"]')
            article = {}
            #next_page = sel.xpath('//div[@class="pages"]/a[text()="%s"]/@href' % u'下一页').extract()[0]

            for a in article_list:
                small_img_location = []
                sent_kafka_message = copy.deepcopy(original_sent_kafka_message)

                article['url'] = urlparse.urljoin(u'http://www.zngonghui.com/index.php',a.xpath('./div/a/@href').extract_first())
                print article['url']
                article['domain'] = 'zngonghui.com'
                article['title'] = a.xpath('./div[2]/div/a/text()').extract_first()
                small_img_url = urlparse.urljoin(u'http://www.zngonghui.com/index.php',a.xpath('./div/a/img/@src').extract_first())
                print small_img_url
    ...
    def parse_detail_page(self, response):
        try:
            ...
            if response.status == 200:
                ...
                pub_time = sel.xpath('//div[@class="wzInfo"]/span[1]/text()').extract_first().split(u'：')[1]
                sent_kafka_message['publish_time'] = self.parse_toutiao_publish_time(pub_time)
                sent_kafka_message['info_source'] = sel.xpath('//div[@class="wzInfo"]/span[2]/text()').extract_first().split(u'：')[1]
                sent_kafka_message['author'] = sel.xpath('//div[@class="wzInfo"]/span[3]/text()').extract_first().split(u'：')[1]


                content_html = sel.xpath('//div[@class="wzCon"]').extract_first()
        ...
        if content_html:
            ...
            for p in p_content_document_list:
                if isinstance(p,bs4.Tag):
                    if p.findChild('img'):
                        img_url = urlparse.urljoin(u'http://www.zngonghui.com',p.img['src'])
