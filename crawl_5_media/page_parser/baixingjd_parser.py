...
class BaixingjdParser(Parser):
    ...
    def parse_list_page(self, response):
        ...
        try:
            ...
            if response.url == 'http://www.baixingjd.com/news/':
                details = response.xpath('//div[@class="nr_left"]/div')
                for detail in details:
                    sent_kafka_message = copy.deepcopy(original_sent_kafka_message)
                    sent_kafka_message['url'] = detail.xpath('.//div[@class="col-xs-8 h_biao"]/h3/a/@href').extract()[0]  # 详情页url
                    sent_kafka_message['title'] = detail.xpath('.//div[@class="col-xs-8 h_biao"]/h3//text()').extract()[0]  # 标题

                    sent_kafka_message['tags'] = detail.xpath('./div[@class="col-xs-8 h_biao"]/small/a/text()').extract()
                    sent_kafka_message['desc'] = detail.xpath('.//div[@class="col-xs-8 h_biao"]//p/text()').extract()[0]  # 简介

                    author, publish_time = self.get_info(detail)
                    sent_kafka_message['author'] = author
                    sent_kafka_message['publish_time'] = publish_time

                    try:
                        small_img_url = detail.xpath('.//div[@class="col-xs-4"]//img/@src').extract()[0]
                    ...
                    sent_kafka_message['url_domain'] = 'baixingjd.com'
                    sent_kafka_message['parse_function'] = 'parse_detail_page'
                    yield sent_kafka_message

            else:
                details = response.xpath('//ul[@class="nr_ul"]/li')
                for detail in details:
                    sent_kafka_message = copy.deepcopy(original_sent_kafka_message)
                    sent_kafka_message['url'] =  detail.xpath('./div[@class="nr_ul_one"]/a/@href').extract()[0]  # 详情页url
                    sent_kafka_message['title'] = detail.xpath('./div[@class="nr_ul_two"]//h3//text()').extract()[0]  # 标题

                    sent_kafka_message['author'] = detail.xpath('./div[@class="nr_ul_two"]//span[@class="nr_ul_span_two"][1]/text()').extract()[0].strip()[4:]
                    sent_kafka_message['publish_time'] = detail.xpath('./div[@class="nr_ul_two"]//span[@class="nr_ul_span_two"][2]/text()').extract()[0].strip()[3:]

                    sent_kafka_message['desc'] = detail.xpath('./div[@class="nr_ul_two"]//p/text()').extract()[0]  # 简介

                    sent_kafka_message['tags'] = detail.xpath('./div[@class="nr_ul_two"]//span[@class="nr_ul_span_two"][3]/a/text()').extract()
                    try:
                        small_img_url = detail.xpath('./div[@class="nr_ul_one"]/a/img/@src').extract()[0]
                    except Exception as e:
                        small_img_url = None
                    if small_img_url:
                        small_img_location, check_flag = self.get_small_img_location(small_img_url, sent_kafka_message['publish_time'])
    ...
    def parse_detail_page(self, response):
        try:
            ...
            content_html = response.xpath('//div[@class="show-article"]').extract()[0]
            ...
            sent_kafka_message['authorized'] = response.xpath('//div[@class="sr_bcopyright w660"]//text()').extract()[0]
    ...
    def get_info(self, detail):
        text = detail.xpath('.//div[@class="col-xs-8 h_biao"]/small/text()').extract()[0]
        # 发布者： 百姓家电      时间： 2017-04-24   标签：
        # text_list = re.split(r'[\s]\s*', text)
        author = text[:10].strip()[4:].strip()
        publish_time = text[11:].strip()[3:14].strip()
